  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <div className="flex flex-col items-center gap-3">
        <img src={assets.logo_icon} alt='' className='w-[min(18vw,120px)]' />
        <h1 className="font-semibold text-gray-100 text-2xl sm:text-3xl md:text-4xl lg:text-5xl">
          ChitChat
        </h1>
      </div>
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        <div className='flex flex-col gap-2'>
          {
            currState === "Sign up" ? (<p className='text-sm text-gray-600'>Already have an account ? <span onClick={() => setCurrState('Login')} className='font-medidum text-violet-500 cursor-pointer'>Login here</span></p>) : (<p className='text-sm text-gray-600'>Create an account <span onClick={() => setCurrState('Sign up')} className='font-medidum text-violet-500 cursor-pointer'>Click here</span></p>)
          }
        </div>
